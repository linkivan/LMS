...
public class FileDAOImpl implements FileDAO {
	...
	public FileModel getFileById(int id) {
		...
		try {
			retVal = jdbcTemplate.queryForObject(SQL, new Object[] { id }, new RowMapper<FileModel>() {
				@Override
				public FileModel mapRow(ResultSet rs, int rowNum) throws SQLException {
					FileModel file = new FileModel(rs.getInt("id"), rs.getString("uploader_name"),
			...
			System.out.println("Exception in getFileById :=> " + e.getMessage());
		}
	...
	public FileModel getSyllabusByCourseId(int courseId) {
		FileModel retVal = null;
		String SQL = " SELECT file.id," + "        file.uploader_name," + "        file.file_path,"
				+ "        file.file_name," + "        file.upload_time" + "   FROM file, syllabus "
				+ "  WHERE file.id = syllabus.file_id " + "    AND syllabus.course_id = ?";
