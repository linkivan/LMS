...
public class GradeDAOImpl implements GradeDAO {
	...
	public boolean gradeAssignResponse(AssignResponseModel assignRes) {
		String sql = "UPDATE assignment_response SET grade = ? where user_id = ? and assignment_id = ?";
		jdbcTemplate.update(sql, assignRes.getGrade(), assignRes.getUserId(), assignRes.getAssignmentId());
		return true;
	}
