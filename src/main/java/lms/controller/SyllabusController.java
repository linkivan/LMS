...
public class SyllabusController {
	...
	// private String UPLOAD_DIRECTORY = "course_upload_files";
	// //servletContext.getInitParameter("file-upload");
	...
	public String uploadSyllabus(HttpServletRequest request, HttpServletResponse response,
			@PathVariable("courseId") int courseId, @RequestParam CommonsMultipartFile fileUpload,
		...
		/*
		 * 12/05/2016 Freddy added the controller part
		 */
		...
		/*
		 * try { // parses the request's content to extract file data
		 *
		 * @SuppressWarnings("unchecked") List<FileItem> formItems =
		 * upload.parseRequest(request);
		 *
		 * if (formItems != null && formItems.size() > 0) { // iterates over
		 * form's fields for (FileItem item : formItems) { // process fields
		 * that are not form fields (to handle file input) if
		 * (!item.isFormField()) { syllabus = item; } } } } catch (Exception ex)
		 * { ex.printStackTrace(); request.setAttribute("message",
